...
一般来说，如果聚合方法有一个聚合操作`op`，那么该聚合会产生`v0 op v1 op v2 op ...`，其中的`vi op vi+1`表示函数调用`op(vi,vi+1)`。这个操作应该是满足结合律的，也即是说这个操作和你组合元素的顺序关，`(x op y) op z = x op (y op z)`。这样就允许使用并行流进行有效的聚合。
